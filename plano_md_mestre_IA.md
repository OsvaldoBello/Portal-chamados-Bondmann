...
| `IA_TRIAGEM_ANEXOS_ATIVO` | `false` | **Kill switch estreito da F7** (leitura de imagem/PDF), independente do geral — rollback isolado sem desligar toda a triagem. |
| `IA_TRIAGEM_ANEXOS_MAX_ARQUIVOS_IMAGEM` / `_PDF` | `2` / `2` | Tetos por tipo de arquivo processado por chamado (F7). |
| `IA_TRIAGEM_ANEXOS_MAX_BYTES` / `_QUIMICO` | `8MB` / `25MB` | Teto de bytes por arquivo LIDO pela IA (F7) — independente do teto de upload (até 100MB no Químico); bounda memória/CPU do processamento. |
| `IA_TRIAGEM_ANEXOS_MAX_DIMENSAO_PX` / `_QUALIDADE_JPEG` | `1024` / `78` | Redimensionamento/recompressão de imagem antes do envio (F7). |
| `IA_TRIAGEM_ANEXOS_DETAIL` | `low` | Nível de detalhe de imagem enviado ao modelo (`auto`/`low`/`high`) — controla custo em tokens (F7). Inválido degrada para `low`. |
| `IA_TRIAGEM_ANEXOS_PDF_MAX_PAGINAS` / `_MAX_CHARS` | `5` / `6000` | Extração de texto de PDF (F7) — sem OCR; PDF escaneado é ignorado graciosamente. |
...
- 2026-07-24 · doc IA · F2 ajuste (Seções 2.4, 10.1) · **Limiar de confiança das perguntas
  ...
  "só ALTA pergunta" (mitigação 10.1) mandou as perguntas para a nota interna. Nova env
  `IA_TRIAGEM_PERGUNTAS_CONFIANCA_MINIMA` (default `BAIXA` = comportamento pedido; `ALTA`
  restaura a guarda antiga sem deploy; valor inválido degrada para `ALTA`). `decidir_acao`
  compara por ranking BAIXA<MEDIA<ALTA. Testes da matriz atualizados + casos novos por limiar.
...
| Leitura de anexos (imagem/PDF) pela triagem | ✅ Código implementado (2026-07-30) — **atrás de env, off por padrão** | F7 (nova) | `app/ia/anexos_contexto.py` (novo): imagens (jpg/png) via visão (Pillow redimensiona ao maior lado configurável antes do envio, base64 data URI, `detail` configurável `auto`/`low`/`high` — default `low`, custo previsível) e PDF via extração de texto (`pypdf`, páginas/chars limitados). Vídeo, docx/xlsx/pptx e OCR de PDF escaneado ficam FORA de escopo (dispatch por mime, não só convenção). Download via `AnexosStorage.download()` (novo, mesmo bearer `service_role` já usado em uploads de sistema — `app/storage.py`). Processado UMA vez por execução e reaproveitado nos dois passes do Químico (sem download nem chamada de modelo duplicada — Regra #9); anexos são dado do próprio cliente sobre o próprio caso, não a base sigilosa, então participam também do Passe A (Regra #4 preservada). Kill switch estreito `IA_TRIAGEM_ANEXOS_ATIVO` (default `false`), independente do geral — rollback isolado. Custo auditável automático (`usage.prompt_tokens` já cobre tokens de imagem, zero código novo em `ia_triagens.custo_usd`). Testes novos: `test_ia_anexos_contexto.py` (funções puras com bytes reais, não só mocks — lição do incidente `_MARGEM_ORFAO` de 2026-07-29), `test_storage.py` (`download`), plumbing em `test_ia_triagem.py`/`test_ia_quimico.py` (kill switch, dedupe entre passes). Pendente: ligar em sombra e validar custo/qualidade em produção antes de expandir. |
