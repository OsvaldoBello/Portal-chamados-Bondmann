...
| `IA_TRIAGEM_ANEXOS_ATIVO` | `false` | **Kill switch estreito da F7** (leitura de imagem/documento), independente do geral — rollback isolado sem desligar toda a triagem. **Ligado em produção (Railway) em 2026-07-30.** |
| `IA_TRIAGEM_ANEXOS_MAX_ARQUIVOS_IMAGEM` / `_DOCUMENTO` | `2` / `2` | Tetos por tipo de arquivo processado por chamado (F7). "Documento" cobre PDF/Word/Excel/PowerPoint — mesmo perfil de custo. |
| `IA_TRIAGEM_ANEXOS_MAX_BYTES` / `_QUIMICO` | `8MB` / `100MB` | Teto de bytes por arquivo LIDO pela IA (F7) — independente do teto de upload; bounda memória/CPU do processamento. Químico igualado ao teto de upload (pedido do gestor 2026-07-30). |
...
| `IA_TRIAGEM_ANEXOS_PDF_MAX_PAGINAS` | `5` | Páginas de PDF lidas (F7) — só PDF tem esse conceito; Excel/PowerPoint param ao bater o teto de caracteres. |
| `IA_TRIAGEM_ANEXOS_DOCUMENTO_MAX_CHARS` | `6000` | Caracteres extraídos por arquivo de documento (F7, PDF/Word/Excel/PowerPoint) — sem OCR; documento escaneado/sem texto é ignorado graciosamente. |
...
| Migration `ia_triagens` + RLS | ✅ Implementado + **aplicado em produção** (2026-07-22) | F0 | `supabase/migrations/0050_ia_triagens.sql` (C1 conferida). Aplicada via MCP no projeto `<project-ref>` após conferir pré-condições; pós-check: RLS on, 1 policy (`ia_triagens_select_staff:SELECT`), zero de escrita; advisors sem alerta novo. e2e em `tests/e2e/test_rls_ia_triagens.py` (marker `rls`). |
