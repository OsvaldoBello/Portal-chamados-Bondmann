...
| Deploy Vercel (serverless FastAPI) | 🟡 Preparado | 6 | `pyproject [tool.vercel] entrypoint`, `vercel.json` (buildCommand do CSS), `.vercelignore`, init **lazy/idempotente** (pool/CSRF/JWT/Supabase/Storage) para cold start sem lifespan, `app.css` versionado como fallback. Requer env vars no painel Vercel. `⚠️` stack é melhor servida no Railway (Dockerfile pronto). |
