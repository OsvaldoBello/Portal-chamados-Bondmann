...
| Teste de isolamento multi-tenant | ✅ **Validado e2e live** | 2 | Simulação de claims (`SET LOCAL ROLE` + `set_config`) contra o Supabase live: RH só vê o setor RH; TI vê tudo; autor CLIENTE não recebe nota interna. Também validado pelo fluxo e2e completo (abertura→chat→pausa→resolução→avaliação→relatório TI). |
...
| Deploy Vercel (serverless FastAPI) | ✅ **No ar** | 6 | Git-integration → função Python serverless (`/health` 200 com CSP/headers). **Build fix:** deps declaradas em `pyproject.toml` `[project.dependencies]` (o builder **uv** da Vercel ignora `requirements.txt`); `framework=fastapi`; init lazy/idempotente. **Pendente do usuário:** env vars no painel (SUPABASE_*, DATABASE_URL, SESSION/CSRF, ENVIRONMENT=production) + opcional desativar Deployment Protection. `⚠️` servidor persistente é mais natural no Railway (Dockerfile pronto). |
