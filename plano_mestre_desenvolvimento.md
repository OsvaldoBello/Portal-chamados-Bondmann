...
- **DECISÃO:** **Signed URL com TTL de 1 hora (3600s)**, **regenerada a cada renderização** do anexo no backend (geração *on-demand*). **Proibido** cachear a URL assinada além do TTL (em sessão, cookie, Alpine state ou memória de aplicação). Padrão: o template recebe a URL assinada já no fragmento HTML servido; ao recarregar o fragmento (HTMX), gera-se nova URL. Ver Seção 3.9.
  - **🔁 `[HARDENING 2026-08-10]`** — essa decisão levou a um bug real: o polling de `mensagens_fragmento` responde 304 (ETag, Seção 2.2) quando nada muda no chat, então a signed URL embutida no HTML **não era regenerada** enquanto a aba ficava aberta — expirava "atrás das costas" do usuário depois de 1h. Ao mesmo tempo, a signed URL expunha o token do Storage na barra de endereço/histórico do navegador. As duas coisas foram resolvidas juntas: o link do anexo deixou de ser a signed URL do Supabase e passou a ser uma rota própria do backend (`GET /anexos/{chamado_id}/{nome_objeto}`, `app/routes/common.py::baixar_anexo`) que baixa o conteúdo sob RLS e o serve direto — sem nunca expor URL/token do Supabase, e sem depender de o fragmento ter sido re-renderizado recentemente (o link é estável, não expira). Ver `docs/CHANGELOG.md` 2026-08-10.
...
trigger. (A `0090` amplia o *sujeito* dessa trava: ela deixou de valer só para o papel
`CLIENTE` e passou a valer para quem age como AUTOR — o `cliente_id` da linha que não é
staff do setor de destino, ex.: um líder de outro setor que abriu chamado pro TI.) Sem isso, um funcionário apontaria o próprio chamado resolvido para qualquer
...
| Migrations base (enums + 7 tabelas + índices) | ✅ Implementado | 1–2 | Schema canônico Seção 5. Migrations `0001_init_enums` + `0002_tables_core` aplicadas no projeto `<project-ref>` e versionadas em `supabase/migrations/`. RLS habilitado na Fase 2 (`0004`). |
...
| **Automação de acessos (RH→TI) — F1 formulários + F2 fila/gate/API** | ✅ F1 e F2 em código · migration `0091` aplicada em produção · F3 (worker) a seguir | frente própria | Governada pelo [`plano_md_mestre_automacao_acessos.md`](plano_md_mestre_automacao_acessos.md) (detalhe lá — não duplicar aqui). Decisões do gestor (2026-09-14): worker no Railway + Tailscale (D1), gate humano do TI antes de executar (D2), layout só para autores RH/TI/ADMIN (D3), credenciais na mensagem de encerramento ao RH (D4). F1: motor genérico `app/domain/campos_dinamicos.py` (extraído do Químico, com campo condicional), layouts `app/domain/formularios_acessos.py`, registro `app/domain/formularios_dinamicos.py`; cascade de campos por categoria E subcategoria. F2: `automacao_jobs` (0091), card no atendimento com gate do TI, `/api/automacao/*` (pull, token), efeitos do resultado (conta no portal, mensagem ao RH, RESOLVIDO, REVOGAR_LICENCA), vigilância no lifespan; contrato em `docs/automacao_api.md`. Próximo: F3a (túnel Tailscale — infra) e F3 (worker no Railway). |
...
| **Frente de IA de triagem (TI + Químico) — F0–F5 + F7 (anexos) + F8 (reclassificação)** | ✅ F0–F3/F5/F7/F8 em produção ou código completo · F1/F2/F4 atrás de env (gestor) | frente própria | Governada pelo [`plano_md_mestre_IA.md`](plano_md_mestre_IA.md) (detalhe lá — não duplicar aqui). F0–F3: fundação, motor de triagem, perguntas/re-triagem e busca de semelhantes (FTS), todos com migration aplicada em produção. F4 (agente Químico, dois passes) e F5 (red team — **zero vazamentos**) entregues; F4 liga por env. F7: leitura de anexos (imagem/documento) **ligada em produção**. **F8 (2026-08-04, [ADR-0008](docs/adr/0008-ia-reclassifica-categoria.md)):** a triagem passou a poder TROCAR categoria/subcategoria automaticamente quando a divergência é evidente — reversão parcial e registrada da Regra de Ouro #3 (prioridade/status continuam só sugestão). Kill switch próprio (`IA_TRIAGEM_RECLASSIFICACAO_DEPARTAMENTOS`, default vazio); destino restrito ao catálogo do departamento do próprio chamado; Dpto Químico fora por construção (prompt do Passe A intocado). Sem migration. **Pendente do gestor:** envs no Railway para sombra (F1/F2), Dpto Químico na lista de departamentos (F4) e `IA_TRIAGEM_RECLASSIFICACAO_DEPARTAMENTOS=TI` (F8). |
...
| Portal do Cliente — dashboard + abertura + detalhe | 🟡 Em produção | 3 | Rotas `/portal` (FastAPI/Jinja2), `asyncpg`+RLS, tokens da marca. Anexos ✅; abertura com **departamento de destino** ✅; **filtros do histórico "Meus chamados"** (busca em código/assunto/descrição, status e período de abertura — cartões viram o seletor de status e contam dentro do recorte, 2026-08-04) ✅; falta: chat Realtime. |
...
| **🔁 Reabertura do chamado pelo autor** | ✅ Implementado | 3/5 | Migrations `0059` + `0090` (RLS `chamados_update_cliente_reabertura` + trigger). Autor de um `RESOLVIDO` insatisfeito com a solução reabre para `EM_ATENDIMENTO` (`POST /portal/chamados/{id}/reabrir`) — zera avaliação anterior, reabre o chat, registra `REABERTO` no histórico. `PortalService.pode_reabrir` vale mesmo em autoatendimento (não é CSAT). A `0090` corrige o 500 de quem é autor **sem papel `CLIENTE`** (líder/staff de outro setor): o critério da policy é AUTORIA, não papel — ver `docs/CHANGELOG.md` 2026-08-28 e `tests/e2e/test_rls_reabertura_autor.py`. |
...
| **Formulário obrigatório do RH (abertura + conclusão)** | ✅ Implementado | 5 | `app/domain/formularios_rh.py` (2026-08-10, sem migration — mesmo padrão em código do precedente do Químico): 6 subcategorias do RH (Programa de Incentivo à Educação, Aumento de Quadro, Alteração de Função, Encerramento de Contrato, Solicitação de ajuda de custo, Registros de Treinamentos) exigem um anexo (o FB oficial preenchido). Modelos em branco em `app/static/formularios/rh/` (estático público); download exibido na abertura (subcategoria escolhida), no detalhe do Portal e no atendimento do Workspace enquanto pendente. **Bloqueio duplo:** `criar_chamado` (`app/routes/portal.py`) recusa abrir o chamado sem o anexo já no POST; `AtendimentoRepo.formulario_pendente` + `mudar_status`/`encerrar` seguem bloqueando a conclusão como defesa em profundidade (cobre reclassificação posterior). Ver `docs/CHANGELOG.md` 2026-08-10. |
...
| Storage privado + signed URLs (TTL 1h) | ✅ Implementado | 3 | Migration `0007_storage_anexos` (bucket privado + RLS path-scoped). `app/security/uploads.py` (10MB, allow-list, magic bytes, sanitização) + `app/storage.py` (REST via httpx com JWT do usuário; `signed_url`/TTL 1h ainda existe, coberto por `test_storage.py`). **🔁 `2026-08-10`:** o link renderizado para o navegador **deixou de ser a signed URL** — vira `/anexos/{chamado_id}/{nome_objeto}` (`app/routes/common.py::baixar_anexo`), que baixa o conteúdo no backend (`AnexosStorage.download`) e o serve direto, sem expor URL/token do Supabase e sem o link expirar numa aba aberta há muito tempo (bug real corrigido nessa mesma sessão — ver `docs/CHANGELOG.md` 2026-08-10). Upload multipart no detalhe; suíte verde. Validação e2e contra Supabase live pendente. |
| Workspace Operador (Kanban/Lista + SLA visual) | ✅ Implementado | 4 | `/workspace`: fila Lista (polling 15s) + Kanban (Sortable.js DnD → HTMX/fetch persiste status) com **indicador de SLA por cor** (`app/domain/sla_visual.py`, verde/amarelo<25%/vermelho<10%-vencido piscante). Tela de atendimento com ações rápidas. Testado (unit + rotas). **🔁 Colunas extras por setor:** Marketing tem `A_FAZER`/`AGUARDANDO_TERCEIROS` (migrations `0024`/`0043`, exclusivas desde `0048`); **TI tem `PROJETOS`** ("Projetos", migration `0057`, 2026-07-24, **aplicada em produção**), status ativo/não-pausado para demanda de projeto sem vínculo de atendimento reativo; **TI e RH têm `RESPOSTA_CLIENTE`** ("Última Interação do Usuário", migrations `0060`/`0061`, 2026-07-24, ainda **não aplicadas em produção**) — automática via trigger em `mensagens`: entra quando a última mensagem pública é de quem abriu o chamado, sai (volta a `EM_ATENDIMENTO`) quando o setor responde. `app/routes/workspace.py::_status_ui()` ramifica por setor. **🔁 Filtros revisados (2026-08-04):** o form de filtros do Kanban/Fila ganhou botão "Filtrar" — só `data-autosubmit` (evento `change`) deixava o filtro de **data** inerte para quem digita a data, e a submissão implícita do Enter não vale em form sem botão com mais de um campo de texto/data; período passou a ser ancorado no dia em **Brasília** (`app/domain/periodo.py::janela_utc`, comparação `timestamptz` em vez de `::date` no fuso da sessão do Postgres) e data inválida/período invertido avisam na tela. Ver `docs/CHANGELOG.md` 2026-08-04 e 2026-08-03. |
...
| Painel Admin + KPIs + Export CSV | ✅ Implementado | 5 | `/admin`: KPIs (total/abertos/resolvidos, **conformidade de SLA**, **CSAT**, **TMA**), gráficos **Chart.js** (status, CSAT 1–5, por departamento, produtividade por operador), **painel "Últimas avaliações"** (feedback qualitativo), **gestão** de departamentos/categorias (criar/ativar-desativar, **só TI**) + planos (leitura), e **`GET /admin/export/csv`** (inclui `avaliacao_comentario`). Testado + fluxo e2e ao vivo. **🔁 Âncora do CSAT (2026-08-03, pedido do usuário):** o CSAT do mês é lido sobre os chamados **resolvidos no mês** (`resolvido_em`), não sobre os avaliados no mês (`avaliacao_em`) — a nota é do atendimento entregue naquele mês, então mês fechado para de mudar quando chega avaliação atrasada. Vale para o KPI, o gráfico de distribuição, o modal de cada nota e "Últimas avaliações" (mesma base nos quatro); `csat_respostas` vira a taxa de resposta sobre `resolvidos`. Ver `docs/CHANGELOG.md` 2026-08-03. **🔁 Relatório visual `.html` (2026-08-04, pedido do usuário):** `GET /admin/export/html` e `GET /admin/marketing/export/html` baixam um arquivo **autocontido** (Chart.js e CSS inline, zero `<script src>`/CDN) com capa, KPIs, gráficos e tabelas do mesmo recorte da tela — abre sem servidor/internet e imprime em PDF. `app/services/export_html.py` + template standalone `admin/relatorio_export.html`; mesma fonte de dados das telas e o mesmo `_filtrar_por_periodo` do `.xlsx` do Marketing. Ver `docs/CHANGELOG.md` 2026-08-04. |
