...
| Painel Admin + KPIs + Export CSV | ✅ Implementado | 5 | `/admin` (restrito a **TI**, `auth_is_ti()`): KPIs (total/abertos/resolvidos, **conformidade de SLA**, **CSAT**, **TMA**), gráficos **Chart.js** (status, CSAT 1–5, por departamento, produtividade por operador), **gestão** de departamentos/categorias (criar/ativar-desativar) + planos (leitura), e **`GET /admin/export/csv`**. Testado. |
